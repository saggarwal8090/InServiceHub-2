...
const seed = async () => {
    try {
        ...
        await db.run('DELETE FROM provider_details');
        ...
        for (const user of users) {
            const result = await db.get(
                ...
                [user.name, user.email, hashedPassword, user.role, user.city, user.phone, user.is_online]
            );
            user.id = result.id;
        }
        ...
        for (const p of providers) {
            ...
            await db.run(
                ...
                [p.id, svc.category, svc.experience, `Experienced ${svc.service} professional in ${p.city}`, rating, reviews, 1]
            );
