    ...
    try {
        ...
        if (role === 'provider') {
            await db.run(
                ...
                [newUser.id, service_category || null]
            );

            if (service_category) {
                await db.run(
                    'INSERT INTO services (provider_id, service_name, category, price) VALUES (?, ?, ?, ?)',
                    [newUser.id, service_category, 'Home Services', 0]
                );
            }
        }

        const token = jwt.sign({ id: newUser.id, role: role }, JWT_SECRET, { expiresIn: '24h' });
        res.json({ token, user: { ...newUser, service_category } });
    } catch (err) {
        console.error(err);
...
app.get('/api/profile', authenticateToken, async (req, res) => {
    try {
        const user = await db.get(
            'SELECT id, name, email, phone, role, city, is_online, created_at FROM users WHERE id = ?',
            [req.user.id]
        );
...
app.put('/api/profile', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const updatedUser = await db.get(
            'SELECT id, name, email, phone, role, city FROM users WHERE id = ?',
            [req.user.id]
        );
...
app.put('/api/toggle-online', authenticateToken, async (req, res) => {
    ...
    try {
        const row = await db.get(
            'UPDATE users SET is_online = CASE WHEN is_online = 1 THEN 0 ELSE 1 END WHERE id = ? RETURNING is_online',
            [req.user.id]
        );
        res.json({ is_online: row.is_online === 1 });
    } catch (err) {
        console.error(err);
        res.status(500).json({ message: 'Server error.' });
    }
